...
def purged_walk_forward_score(
    ...
    for boundary in boundaries:
        ...
        valid_pred = policy_predictions(valid_prob, threshold=0.52)
        ...
        fold_score = 0.45 * auc + 0.25 * bacc + 0.30 * strategy["sharpe_like"]
        fold_scores.append(fold_score)

    return float(np.mean(fold_scores)) if fold_scores else -np.inf


def select_hyperparameters(train_df: pd.DataFrame, features: list[str]) -> dict:
    candidates = [
        {
...
def main() -> None:
    ...
    best_params = select_hyperparameters(fit_df, features)
    ...
    print(f"Round2 Test OVR ROC-AUC: {test_auc:.4f}")
    print(f"Round2 Balanced Accuracy (raw): {test_bacc_raw:.4f}")
    print(f"Round2 Balanced Accuracy (policy): {test_bacc_policy:.4f}")
    print(f"Round2 Strategy Return: {test_stats['return']:.4%}")
    print(f"Round2 Strategy Max DD: {test_stats['max_drawdown']:.4%}")
    ...
    joblib.dump(calibrated, models_dir / "aapl_intraday_round2_calibrated.joblib")
    (models_dir / "aapl_intraday_round2_features.txt").write_text("\n".join(features))
    ...
    (reports_dir / "round2_training_summary.json").write_text(json.dumps(summary, indent=2))
    print("Saved round2 model artifacts and summary.")
